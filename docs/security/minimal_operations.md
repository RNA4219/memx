...
5. `archive_move` / `archive_purge` の監査ログには、成功/失敗を問わず `result`, `reason`, `retryable`, `owner`, `next_attempt_at` を必ず含める。
6. 要件ID `REQ-SEC-AUD-001` / `REQ-SEC-AUD-002` の充足を確認して記録を確定する。
7. バックアップを保持する場合も要件本文と同一保持期間を適用する。
