...
- 保持期間: 正式な日数定義は要件本文（2-7-1）に従う（internal/secret の最小保持 90 日を含む）。
- 削除ルール:
  - 期限超過分は週次で削除（手動または定期ジョブ）。
  - `archive` 退避/物理削除の条件・実行主体・監査ログ必須項目は要件本文（2-7-3 / 2-7-4）を参照する。
  - `archive_move` / `archive_purge` 失敗時の必須記録は要件ID（`REQ-SEC-AUD-001` / `REQ-SEC-AUD-002`）を参照する。
